      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-start justify-between p-4 border-b border-gray-200">
          <div>
            ...
            <h2 className="text-xl font-semibold text-gray-900">Detailed Explanation</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-5">
          {panel.extractedText && (
            <div className="space-y-2">
              ...
              <p className="bg-blue-50 border border-blue-200 text-blue-900 p-3 rounded-lg font-japanese leading-relaxed">
                {panel.extractedText}
              </p>
            </div>
          )}
          ...
          {panel.translation && (
            <div className="space-y-2">
              ...
              <p className="bg-green-50 border border-green-200 text-green-900 p-3 rounded-lg leading-relaxed">
                {panel.translation}
              </p>
            </div>
          )}
          ...
          {panel.context && (
            <div className="space-y-2">
              ...
              <p className="bg-amber-50 border border-amber-200 text-amber-900 p-3 rounded-lg leading-relaxed">
                {panel.context}
              </p>
            </div>
          )}
          ...
          {panel.sentences && panel.sentences.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-lg font-semibold text-gray-900">Sentence Breakdown</h3>
              <div className="space-y-3">
                  ...
                  <div key={index} className="p-3 border border-gray-200 rounded-lg bg-gray-50">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 space-y-1">
                        <p className="text-gray-900 font-japanese font-semibold">{sentence.sentence}</p>
                    ...
                    {sentence.words && sentence.words.length > 0 && (
                      <div className="mt-3 p-3 bg-white rounded-lg border border-blue-100">
                        <p className="text-sm font-medium text-blue-900 mb-2">Vocabulary</p>
                        <div className="grid gap-2">
                          {sentence.words.map((word, idx) => (
                            <div
                              key={idx}
                              className="p-2 rounded border border-gray-200 bg-blue-50 text-sm"
                            >
                              <div className="flex items-center gap-2">
                                <span className="font-semibold text-gray-900 font-japanese">{word.word}</span>
                    ...
                    {sentence.grammar && sentence.grammar.length > 0 && (
                      <div className="mt-3 p-3 bg-white rounded-lg border border-purple-100">
                        <p className="text-sm font-medium text-purple-900 mb-2">Grammar</p>
                        <div className="grid gap-2">
                          {sentence.grammar.map((grammar, idx) => (
                            <div key={idx} className="p-2 rounded border border-purple-200 bg-purple-50 text-sm">
                              <p className="font-semibold text-purple-900">{grammar.pattern}</p>
                              <p className="text-gray-700">{grammar.explanation}</p>
                              {grammar.example && (
                                <p className="text-purple-700 text-xs mt-1">Example: {grammar.example}</p>
                              )}
                            </div>
                          ))}
                        </div>
