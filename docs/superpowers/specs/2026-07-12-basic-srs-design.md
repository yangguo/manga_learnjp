...
- 固定依赖 `ts-fsrs@5.4.1`，集中封装 DTO 转换、四档调度、区间预览和系统时间回拨校验。
- `word-bank-storage` 已升级为 v3；v1/v2 词汇原样迁移，非法或孤立复习卡会被丢弃。
- `/review`、Header 徽标和生词本入口已上线；支持 Space 显示答案及 1-4 四档评分。
- 生词本的持久化变更使用同一浏览器独占锁；durable 写入失败不推进内存状态，跨标签删除会跳过失效卡而不恢复已删除词。
- 浏览器验收覆盖 v2 迁移、损坏本地存储错误态、正反面隐藏、评分持久化、刷新恢复、跨标签删除、完成状态和 375px 两列评分布局。
- 功能提交：`ff1bcdd`、`ba2327e`、`00a67c8`、`9a9115d`、`4e64bf1`、`8e0d16a`。
