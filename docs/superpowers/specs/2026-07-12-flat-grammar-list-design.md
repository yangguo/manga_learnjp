...
- 继续使用 `filterLearningGrammar` 过滤基础 N5 语法，符合现有产品规则。
- 每张语法卡继续显示构式、解释、例句、JLPT 徽标和收藏按钮。
- 不修改全局默认 N4 设置、缓存结构、分析接口或独立生词本页面。

## 实现

在 `src/lib/analysis-order.ts` 增加纯函数，将句子中的语法按原顺序展平后应用现有学习语法过滤。`MokuroAnalysisPanel` 直接渲染返回的平铺数组，并删除仅供等级分组使用的组件、文案和依赖。

## 验证

- 单元测试证明跨句和句内语法顺序保持不变。
- 单元测试证明现有 N5 基础语法过滤仍生效。
