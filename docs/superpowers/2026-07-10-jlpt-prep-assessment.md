...
日期: 2026-07-10
最后更新: 2026-07-12
状态: M4 已完成 / M5 降级完成：语法收藏 / 语法等级数据准入受 Tanos 上游 500 阻塞
...
| Anki 兼容导出 | M3 已完成 | `src/lib/anki-export.ts`、`src/components/WordBank.tsx` | 全部生词可导出为带字段与标签声明的 UTF-8 TSV |
| SRS 复习 | 已完成 | FSRS 四档评分、每日队列和本地持久化 | 后续可在真实使用数据基础上评估统计需求 |
...
| M4:基础 SRS | 已完成 | 使用 ts-fsrs 为已收藏词增加四档复习状态和每日队列 | v3 本地持久化、旧收藏零丢失、四档调度和桌面/移动验收通过 |
| M5:语法等级与收藏 | 降级完成：语法收藏 | Tanos 许可页可审查，但 N1–N5 页面均返回 HTTP 500；独立语法收藏已交付 | 不展示或持久化语法等级；上游恢复并通过数据准入后才能启用固定字典和四组展示 |
...
M3 功能提交:`5684b74`。M4 功能提交:`ff1bcdd`、`ba2327e`、`00a67c8`、`9a9115d`、`4e64bf1`、`8e0d16a`。M4 已通过单元测试、lint、生产构建和浏览器验收。M5 数据准入已探测：Tanos 许可页可审查，但五级 grammar list 端点在 `2026-07-12` 均为 HTTP 500；独立语法收藏已完成并通过桌面/移动验收。后续下一步是在上游恢复后重新执行完整等级数据准入。
